/**
 * @file copy from HtmlPlugin，解决matrix下多套样式的兼容性问题
...
const matrixFns = (data, matrixEnv) => {
    ...
    matrixEnv.forEach(env => {
        ...
        matrixAssets[env] = {bodyAsset, headAsset};
    });

    const html = data.html;

    return html.replace(MATRIX_SMARTY_BLOCK, (block, quote, env, tag, content) => {
        const asset = matrixAssets[env] && matrixAssets[env][`${tag}Asset`] && matrixAssets[env][`${tag}Asset`].join('');
        return asset ? block.replace(content, asset) : block;
    });
};

const main = (pluginData, compilation) => {
...
const name = 'HulkHtmlWebpackPlugin';
