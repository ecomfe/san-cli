...
function formatMessage(message) {
    ...
    // 添加对 at Generator.next (<anonymous>)的处理
    message = message.replace(/^\s*at\s((?!webpack:).)*(?:\:\d+\:\d+|<anonymous>)[\s)]*(\n|$)/gm, ''); // at ... ...:x:y
...
module.exports = class SanFriendlyErrorsPlugin {
    apply(compiler) {
        const done = stats => {
            ...
            if (hasErrors) {
                clearConsole();
                error(chalk.red(`Failed to compile with ${messages.errors.length} errors.`));
                showError(messages.errors);

            }
            if (hasWarnings) {
                warn(`Compiled with ${messages.warnings.length} warnings.`);
                showError(messages.warnings);
            }
        };
